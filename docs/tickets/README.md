...
**Ticket ID prefix:** `MM` (e.g., `MM-T001`, `MM-T002`, ...)

> **Note:** This repo also uses the `dev-cycle` plugin (state lives in `.workflow/`) for its 4-team continuous-improvement loop, and Architecture Decision Records in `docs/adrs/`. Tickets **complement** those — `.workflow/` tracks the automated cycle, `docs/adrs/` records architecture decisions, and tickets here track discrete units of human-directed work. When a ticket leads to an architecture decision, log an ADR and link it; when a dev-cycle surfaces work worth tracking, open a ticket.
...
docs/
  ...
  adrs/                 # Architecture Decision Records (this repo's ADR home)
    0001-*.md
  decisions/            # (ticketing-plugin default ADR location, if used)
    INDEX.md
    0001-*.md
...
# → docs/tickets/open/MM-T001-add-mfa-to-login-flow.md

# Start work
/ticketing:move MM-T001 in-progress

# Finish
/ticketing:move MM-T001 done
...
- **IDs are immutable.** Once a ticket is created as `MM-T042`, it keeps that ID forever, even across state changes.
...
```yaml
---
id: MM-T001                       # immutable, auto-generated
...
created: 2026-06-03               # auto-set on creation
updated: 2026-06-03               # auto-bumped on move
closed: 2026-06-20                # auto-set on move → done
related-pr: https://...           # optional
related-tickets: [MM-T005]        # optional
---
```
...
- **Filename:** `<ID>-<slug>.md` — e.g., `MM-T001-add-mfa-to-login.md`.
- **Slug:** lowercase, hyphenated, 40 chars max.
- **ID format:** `MM-T<3-digit-zero-padded>` for tickets, `<4-digit>-<slug>` for ADRs, `YYYY-MM-DD-<slug>` for lessons.
